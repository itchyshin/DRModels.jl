...
def public_files(root: Path, *, public_only: bool) -> list[Path]:
    return sorted(
        ...
        and (not public_only or "developer-notes" not in path.relative_to(root).parts)
    )
        ...
        for number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            for kind, pattern in PATTERNS.items():
                if pattern.search(line):
                    problems.append(f"{path.relative_to(root)}:{number}: {kind}: {line.strip()}")
    return problems
...
def main() -> int:
    ...
    parser.add_argument("--public-only", action="store_true", help="compatibility flag; all scanned files are public sources")
    ...
    if problems:
        ...
        print("\n".join(problems))
        return 1
    print("READER SURFACE AUDIT PASSED")
    return 0
