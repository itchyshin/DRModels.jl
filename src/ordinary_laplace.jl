...
# equal to the group index. Two differences from those routes, both so that
# value and gradient describe TMB's function everywhere: the random-effect
# log-SD and the dispersion are unclamped (`raw_scales = true`).
#
...
_ordinary_laplace_scale_setup(::NegBinomial2, y, Xμ) =
    (Val(:nb2_fixed), _nb2_laplace_setup(y, Xμ; raw_scales = true)...)
