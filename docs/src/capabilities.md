...
| One `phylo`/`relmat`/`animal` marker **plus** ordinary `(1\|h)` or `(0 + x\|h)` bars on the mean | tree / `K` / `A` | **Tested**, ML only; matches drmTMB `engine = "tmb"` on nine fixtures (`docs/dev-log/evidence/arc2-structured-ordinary-bar/`). Independent blocks; the marker SD is on the correlation scale. REML, `(1 + x\|h)`, range-estimated `spatial()`, `meta_V()`, `penalty` and sparse algorithms are refused by name. |
